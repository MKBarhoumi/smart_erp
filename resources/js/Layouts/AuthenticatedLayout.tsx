...
export default function AuthenticatedLayout({ children }: PropsWithChildren) {
  ...
  return (
    <div className="min-h-screen flex bg-gray-50">
      ...
      <aside className={`fixed inset-y-0 left-0 z-30 w-64 bg-slate-900 transform transition-transform duration-200 lg:translate-x-0 overflow-y-auto ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center h-16 px-6 border-b border-slate-800">
          <Link href="/dashboard" className="text-xl font-bold text-white tracking-tight">
            Nvo<span className="text-indigo-400">ERP</span>
          </Link>
        </div>
      ...
      {/* Main Content - Add left margin for fixed sidebar on large screens */}
